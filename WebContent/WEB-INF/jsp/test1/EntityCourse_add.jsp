	    	...
	    	$("ul",_win).tree({
	    		...
	    		//	alert(node.state);
	    			if(node.state=="open"){			
		    			...
		    			$("#classCid").attr("style","width: 120px;height:30px;visibility:visible");
		    			$(_win).window('close');
	    			}
...
<div class="page-container" >
	<form id="classAddForm" class="form form-horizontal" method="post" action="${pageContext.request.contextPath }/classes/addclass">	    
	    <div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
	            ...
	            <input type="text" name="classCid" id="classCid"   style="width: 120px;height:30px;visibility:hidden"  readonly ></input>
			</div>
		</div>    
	    <div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">课程题目：<span class="c-red">*</span></label>
			<div class="formControls col-xs-8 col-sm-9">
				<input class="easyui-textbox input-text"   type="text" name="classTitle" id="classTitle" value="${classes.classTitle }" data-options="required:true" style="width:180px;"></input>
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">课程卖点：<span class="c-red">*</span></label>
			<div class="formControls col-xs-8 col-sm-9">
				<input class="easyui-textbox input-text" name="classSellPoint" id="classSellPoint" value="${classes.classSellPoint }" data-options="multiline:true,validType:'length[0,150]'" style="width: 180px;"></input>
			</div>
		</div>	
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">课程价格：<span class="c-red">*</span></label>
			<div class="formControls col-xs-8 col-sm-9">
				<input class="easyui-numberbox input-text" type="text" name="classPrice"  id="classPrice" value="${classes.classPrice }"  data-options="min:1,max:99999999,precision:2,required:true" tyle="width: 180px;"/>
			</div>
		</div>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" placeholder="报名起始时间" onfocus="WdatePicker({ dateFmt:'yyyy-MM-dd HH:mm:ss',maxDate:'#F{$dp.$D(\'datemax\')||\'%y-%M-%d\'}' })" id="datemin" class="input-text Wdate" style="width:180px;">
				--
				<input type="text" placeholder="报名结束时间" onfocus="WdatePicker({ dateFmt:'yyyy-MM-dd HH:mm:ss',minDate:'#F{$dp.$D(\'datemin\')}' })" id="datemax" class="input-text Wdate" style="width:180px;">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">开课时间：<span class="c-red">*</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" placeholder="报名起始时间" onfocus="WdatePicker({ dateFmt:'yyyy-MM-dd HH:mm:ss',maxDate:'#F{$dp.$D(\'datemax\')||\'%y-%M-%d\'}' })" id="datemin" class="input-text Wdate" style="width:180px;">
				--
				<input type="text" placeholder="报名结束时间" onfocus="WdatePicker({ dateFmt:'yyyy-MM-dd HH:mm:ss',minDate:'#F{$dp.$D(\'datemin\')}' })" id="datemax" class="input-text Wdate" style="width:180px;">
